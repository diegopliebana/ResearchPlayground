...
output = "../exp/banditEAOneMax_div_dim.pdf"
...
for i in range(num_algs):
    avg_block_dv_dim = np.divide(all_averages[i],steps[0:len(all_averages[i])])
    stderr_block_dv_dim = np.divide(all_stderr[i],steps[0:len(all_averages[i])])
